...
// 0: Welcome
// 1: API key
// 2 (Mac only): Permissions
// 2 / 3: Hotkey setup
// 3 / 4: Help improve
const SETUP_STEP  = IS_MAC ? 3 : 2
const HELP_STEP   = IS_MAC ? 4 : 3
const TOTAL_STEPS = IS_MAC ? 5 : 4
...
export function Onboarding() {
  ...
  const handleFinish = () => {
    trackEvent('onboarding_completed', { steps_completed: String(TOTAL_STEPS) })
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="w-[60%] bg-surface flex items-center justify-center px-16 overflow-hidden">
        <div className="max-w-[480px] w-full">
          ...
          {step === 1            && <ApiKeyStep      onNext={() => setStep(2)} />}
          {IS_MAC && step === 2  && <PermissionsStep onNext={() => setStep(3)} />}
          {step === SETUP_STEP   && <SetupStep       onNext={() => setStep(HELP_STEP)} />}
          {step === HELP_STEP    && <HelpImprove     onNext={handleFinish} />}
        </div>
      </div>
    </div>
  )
