  ...
  useEffect(() => {
    ...
    async function stopAndProcess() {
      try {
        ...
        // Temp: write raw error to debug element
        const dbg = document.getElementById('_dbg_err') || (() => { const d = document.createElement('div'); d.id = '_dbg_err'; d.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.9);color:#f66;font-size:11px;padding:8px;z-index:99999;overflow:auto;white-space:pre-wrap;word-break:break-all'; d.onclick = () => d.remove(); document.body.appendChild(d); return d })(); dbg.textContent = errMsg
        if (errMsg.includes('transcription_failed') || errMsg.includes('gemini_error')) {
