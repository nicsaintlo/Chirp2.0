...
pub async fn start_recording(
    ...
    // Check model is loaded (skip when using the Google/Gemini cloud backend)
    let using_google = s.settings.ai_backend == "google" && !s.settings.google_token.is_empty();
    if s.recognizer.is_none() && !using_google {
...
pub async fn stop_recording(
    ...
    let (recognizer_opt, smart_fmt, dict, snips, ai_cleanup, llm_port, tone_mode, ai_backend, google_token) = {
        let s = state.lock().await;
        let using_google = s.settings.ai_backend == "google" && !s.settings.google_token.is_empty();
        let rec = if !using_google {
            Some(s.recognizer.clone().ok_or("model_not_loaded".to_string())?)
        } else {
            None
        };
        (
            ...
            s.settings.ai_backend.clone(),
            s.settings.google_token.clone(),
        )
    };

    let is_google = ai_backend == "google" && !google_token.is_empty();

    let had_dictionary = !dict.is_empty();

    // Transcribe audio — either via Gemini multimodal API (cloud) or the local
    // Parakeet TDT model (Sherpa-ONNX).  Both paths apply the same
    // regex/dict/snippet post-processing so downstream code is identical.
    let result: Result<String, String> = if is_google {
        log::info!("Using Gemini for transcription...");
        match crate::gemini::transcribe_audio(&google_token, &audio_data).await {
            Ok(raw) if raw.is_empty() => {
                log::warn!("Gemini transcription returned empty text");
                Err("transcription_failed".to_string())
            }
            Ok(raw) => {
                log::info!("Gemini STT: '{raw}'");
            ...
            Err(e) => {
                log::error!("Gemini transcription failed: {e}");
                Err(format!("gemini_error: {e}"))
            }
        }
    } else {
        // Local path: chunk long audio into 30s segments so the 0.6B model
        // stays in its comfort zone — longer buffers produce gibberish.
        let recognizer = recognizer_opt.unwrap();
    ...
    // AI cleanup pass — Gemini/Gemma cloud or local llama-server, depending on backend
    let mut was_cleaned_up = false;
    let after_llm = if ai_cleanup && is_google {
        let _ = app_handle.emit("recording-state", "polishing");
        log::info!("Running Gemma cleanup on text...");
        match crate::gemini::cleanup_text(&google_token, &formatted, &tone_mode).await {
            Ok(cleaned) => {
                log::info!("Gemma cleanup: '{cleaned}'");
                was_cleaned_up = true;
                cleaned
            }
            Err(e) => {
                log::warn!("Gemma cleanup failed, using regex-only result: {e}");
                formatted.clone()
            }
        }
    } else if ai_cleanup && llm_port.is_some() {
        let port = llm_port.unwrap();
        let _ = app_handle.emit("recording-state", "polishing");
        log::info!("Running local LLM cleanup on text...");
...
/// Returns whether this build was compiled with Google OAuth credentials.
#[tauri::command]
pub fn get_oauth_configured() -> bool {
    crate::oauth::is_configured()
}

/// Start the Google OAuth PKCE flow. Returns the authorization URL to open
/// in the browser. A background task listens for the localhost callback,
/// exchanges the code for a token, creates an API key, and emits
/// `google-auth-complete` (or `google-auth-error`) when done.
#[tauri::command]
pub async fn start_oauth_login(app_handle: AppHandle) -> Result<String, String> {
    crate::oauth::start_flow(app_handle).await
}
